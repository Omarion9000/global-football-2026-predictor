...
A deterministic statistical engine (Dixon-Coles with confederation-strength correction, seeded Monte Carlo simulation over the 48-team bracket) produces title probabilities, group-stage advancement, and a knockout tree that a Next.js App Router UI surfaces as three broadcast-pastel views — available in English and Spanish via an instant in-page toggle. Engine, scheduler, persistence, and UI are isolated by lint boundaries and verified by 690+ tests.
...
| 8B        | ✅ Dixon-Coles candidate v0.2 (backtest-only). Weighted MLE with analytic gradients + backtracking + ridge; lazy-refit predictor with strict no-lookahead. Leakage-safe tuning on 2016-17 + 2017-18; headline on 2018-19..2024-25. Beats both naive baselines on Brier and log-loss; loses to closing-line market by ~1pp Brier (documented). See [`docs/16`](docs/16_MODEL_V02_CANDIDATE.md). **Production stays `MODEL_VERSION = "v0.1.0"`** — promotion is a separate future phase requiring its own approval. |
| 8D        | ✅ Real EPL data foundation. football-data.org client (server-only, 429 retry, opaque errors), canonical team map covering all 34 corpus clubs + current PL, sync + backfill scripts. Migration `0002_add_league_stage.sql` widens `fixtures.stage` to accept `'LEAGUE'`. **Production cron still schedules from the mock catalog; UI unchanged.** See [`docs/17`](docs/17_REAL_DATA_PIPELINE.md). |
| 9A        | ✅ National-team data foundation. martj42/international_results corpus (CC0), top-tier 15-tournament filter, 223-nation canonical map (every confederation), idempotent loader. Migration `0003_add_international_data.sql` adds `'INTERNATIONAL'` stage + `tournament` column + drops `teams.code` UNIQUE. **Production cron + UI unchanged.** See [`docs/18`](docs/18_INTERNATIONAL_DATA.md). |
...
| **9D.1**  | ✅ **EN / ES language toggle.** Central dictionary (`src/i18n/dictionary.ts`) keyed by namespaced string IDs with both languages. Cookie-persisted (`lang`, 1-year max-age) and read server-side in the layout so the first HTML payload already carries the user's preferred language. Toggle in the masthead switches all UI chrome instantly without reload. Country names + confederation codes + technical terms (Monte Carlo, Dixon-Coles, α, δ) stay in source form; round names use proper Mexican-Spanish football vocabulary (dieciseisavos / octavos / cuartos / semifinal / final). Dictionary parity test guarantees no half-translated UI. See [`docs/21`](docs/21_UI.md) §10. **UI-only; no engine / schema / model changes.** |
| **9D.2**  | ✅ **Live-state UI — Matchday 1 results pinned.** Re-ran `pnpm sim:ui` with all 24 Matchday-1 results in `data/tournament/results.json`. The committed simulator JSON (`src/data/tournament-sim.json`) now carries `playedMatches: 24` in `meta`; the home view conditionally swaps the kicker chip ("Pre-tournament prediction" → "24 matches played · live" / "24 partidos jugados · en vivo"), the hero subtitle, and the dot colour (peach → deep sage) when `playedMatches > 0`. Reverting `results.json` to empty + re-running `sim:ui` restores the pre-tournament copy automatically — same code path. Post-MD1 top six: Spain 18.6 %, Brazil 15.9 %, England 9.0 %, Argentina 8.8 %, Germany 8.1 %, Portugal 6.3 %. See [`docs/21`](docs/21_UI.md) §11. **UI / data-bridge only; no engine / schema / model changes.** |
| **9E**    | ✅ **FIFA 2026 knockout bracket structure (Option 1a).** Replaced the placeholder R32 tree with FIFA's published 2026 bracket: exact 1A..1L / 2A..2L pairings for M73–M88, exact R16 → Final feed table for M89–M104. Third-place slots carry their real FIFA cluster (5-group eligibility set); a new `thirdPlaceAssignment.ts` module runs augmenting-path bipartite matching per simulation pass with a deterministic best-effort fallback, counted by the simulator. **Observed fallback rate on the post-MD1 corpus: 0 / 10 000 = 0.00 %** — Option 1a is structurally exact for this state; FIFA's 495-scenario Annex C table is intentionally not enumerated. Post-bracket title odds reshuffle: Spain 16.7 %, Argentina 11.6 %, England 11.5 %, Brazil 10.9 % (Brazil drops 5 pp because 1C vs 2F is a real heavyweight R32). See [`docs/20`](docs/20_TOURNAMENT_SIMULATOR.md) §4.4. **Simulator + UI + docs only; engine math, MODEL_VERSION, schema, persistence, cron all untouched.** |
| **9F**    | ✅ **Host home advantage in the group stage.** Mexico, Canada, and the USA receive the fit's measured `homeAdv` term in their group-stage matches (verified: every host plays all three group fixtures on home soil in 2026). Knockout matches stay neutral — host knockout venues span all three countries and depend on bracket path, so that limitation remains and is surfaced honestly in the methodology card. Implementation is config-driven (`HOST_NATIONS` set + `neutral?: boolean` threaded through the `MatchEngine` interface) — no per-match venue table. **Dixon-Coles math is byte-identical** to Phase 9B/9B.2; `MODEL_VERSION` stays `'v0.1.0'`. Group-stage effects (Mexico already group-dominant: sharper P(1st) only; USA: +2.1 pp R16, now clear Group D favourite over Australia; Canada: +3.9 pp R16, clear Group B runner-up). Non-host title odds shift <±0.3 pp — within Monte-Carlo noise at N = 10 000. See [`docs/20`](docs/20_TOURNAMENT_SIMULATOR.md) §5. **Simulator + UI + docs only; engine math, schema, persistence, cron untouched.** |
| **9F.1**  | ✅ **Strict `results.json` validation — fail loud, not silent.** A new `validateResults` module gates the runner before the 12-second fit: it checks `stage`, canonical team names against this tournament's roster, `home ≠ away` (post-resolution so aliases of the same team count), non-negative integer goals, group-pair orientation against `defaultGroupSchedule` (with an explicit "swap home/away" hint when the reverse orientation would have matched), knockout-stage decisiveness, and duplicate `(stage, home, away)` entries. Each error names `results[N]` and the actionable fix. **The validator immediately caught a real bug**: 5 of the 24 Matchday-1 entries had inverted home/away vs the schedule and had been silently re-rolled from the model in every run from Phase 9D.2 onward (the UI's "24 matches played" was correct by JSON count but wrong by effective pin count — only 19 were truly fixed). The orientation fix is bundled into this commit; biggest corrections were Sweden P(R16) +12.4 pp and Colombia P(R16) +7.5 pp once their lopsided MD1 wins were actually pinned. Runs in <5 ms. See [`docs/20`](docs/20_TOURNAMENT_SIMULATOR.md) §4.7. **Validator + tests + bilingual copy polish; engine math, schema, persistence, cron, `MODEL_VERSION` all untouched.** |
...
- **Test discipline.** 720+ tests covering engine, simulation, persistence, scheduler, the national-team backtest harness, the offline tournament simulator (including FIFA bracket structural assertions cell-by-cell, the third-place cluster matching, host home-advantage wiring, and strict `results.json` validation), the bilingual EN/ES UI, and the data-bridge contract — including property-based engine checks, append-only constraint tests, dictionary parity (no half-translated UI possible), flag-coverage, and UI-vocabulary scans that prove restricted marks aren't in the deployed product.
