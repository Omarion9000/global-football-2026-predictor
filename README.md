...
- **Test discipline.** 700+ tests covering engine, simulation, persistence, scheduler, the national-team backtest harness, the offline tournament simulator (including FIFA bracket structural assertions cell-by-cell and the third-place cluster matching), the bilingual EN/ES UI, and the data-bridge contract — including property-based engine checks, append-only constraint tests, dictionary parity (no half-translated UI possible), flag-coverage, and UI-vocabulary scans that prove restricted marks aren't in the deployed product.
